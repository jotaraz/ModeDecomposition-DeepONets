#!/bin/bash
# Produce the joint GD+Adam with/without-sigma mode-loss figure on a COMPUTE NODE (never the login node).
set -uo pipefail
...
python -m src.analysis.RELEVANT.plot_modelosses_with_out_sigma_joint
echo "EXIT=$?"
