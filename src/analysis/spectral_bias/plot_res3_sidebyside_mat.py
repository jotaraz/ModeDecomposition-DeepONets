...
for irow, sigmascale_num in enumerate([-0.01, -0.5]):
    ...
    for d in direcs:
        if d in alldirs:
            ...
            axs[irow, 0].plot(epochs, train_loss, '--', color=colors[k])
            if len(labels) == len(direcs):
                tag = labels[k]
            
            axs[irow, 0].plot(epochs, test_loss, '.-', color=colors[k], label=tag+" Loss")
            ...
            for i in range(0, num_saves, 1):
                ...
                axs[irow, 1+k].plot(mode_data[i, 1:1+llw],'--',color=(0.2 + 0.7*i/num_saves, 0, 0))
            ...
            axs[irow, 1+k].set_yscale("log")
            if irow == 0:
                axs[irow, 1+k].set_title(tag, color=colors[k])
            print(d, num_saves)
    ...
    for kk in range(k):
        ...
        axs[irow, 1+kk].set_zorder(1)  
        axs[irow, 1+kk].patch.set_alpha(0.0)     # make background transparent so image shows through
        
        axs[irow, 1+kk].set_ylim((0.5*minmode, 2*maxmode))

    axs[irow, 0].set_xlabel("Epochs")
    axs[irow, 0].set_ylabel("Loss")
    if irow == 0:
        axs[irow, 2].plot([], [], '--', color=(0.2, 0, 0), label="Initial Mode Losses")
        axs[irow, 2].plot([], [], '--', color=(0.2+0.7, 0, 0), label="Final Mode Losses")    
        axs[irow, 2].plot([], [], 'o-', color=colors[0], linewidth=2, label=labels[0])    
        axs[irow, 2].plot([], [], 'o-', color=colors[1], linewidth=2, label=labels[1])    
        #axs[irow, 0].legend()

        axs[irow, 2].legend(loc='lower left', bbox_to_anchor=(-2.0, 1.1), framealpha=1.0, ncol=2)


plt.subplots_adjust(top=0.75) #right=0.7)
