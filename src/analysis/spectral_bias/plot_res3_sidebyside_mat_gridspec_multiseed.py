...
## ---------------------------------------------------------------------------
## MULTISEED VARIANT of plot_res3_sidebyside_mat_gridspec.py
## Every plotted point is the ARITHMETIC MEAN over seeds v0..v9 of the
## synthv09082026 nets.  log.txt stores log10 values and this figure plots
## 10**(col/2), so each seed is converted FIRST and averaged afterwards --
## averaging the raw column would give a geometric mean.  log_modes.txt stores
## raw values and is averaged directly.
## Bands (mean +/- 1 std) are drawn when MULTISEED_BAND=1; see multiseed.py.
## ---------------------------------------------------------------------------
from ..RELEVANT import multiseed
...
for irow, sigmascale_num in enumerate([-0.01, -0.5]):
    ...
    for d in direcs:
        if d in alldirs:
            loss_data, _, _, nseed = multiseed.load_mean(d, "log.txt")
            if loss_data is None:
                print("multiseed: no usable seed for", d); continue
            print("multiseed: %s -> mean over %d seeds" % (d, nseed))
            de = 2
            epochs = loss_data[::de,0]
            # arithmetic mean of the PLOTTED error: convert per seed, then average
            _tr, _tr_lo, _tr_hi, _ = multiseed.load_mean(
                d, "log.txt", transform=lambda a: 10**(a[::2,1]/2))
            _te, _te_lo, _te_hi, _ = multiseed.load_mean(
                d, "log.txt", transform=lambda a: 10**(a[::2,4]/2))
            # kept as log10 so the existing 10**(x/2) plotting below still works
            train_loss = 2*np.log10(_tr)
            test_loss  = 2*np.log10(_te)
            
            tag = d.split("m5000_")[1].split("_numd5")[0]
            
            multiseed.band(axs[irow], epochs, _tr_lo, _tr_hi, colors[k])
            multiseed.band(axs[irow], epochs, _te_lo, _te_hi, colors[k])
            ...
            mode_data, _md_lo, _md_hi, _ = multiseed.load_mean(d, "log_modes.txt")
            num_saves = np.shape(mode_data)[0]
            for i in range(0, num_saves, 1):
                #print(i, 3.0*i/num_saves)
                if multiseed.is_endpoint_stage(i, range(0, num_saves)):
                    multiseed.band(axs[2*(k+1)+irow], np.arange(llw),
                                   _md_lo[i, 1:1+llw], _md_hi[i, 1:1+llw],
                                   generate_flow_color(i/num_saves))
...
tmp = "Fig6_multiseed" + multiseed.suffix()
