...
for ib, bid in enumerate(bids):
    ...
    nt, nb, rtrain, rtest, ptrain, ptest, utrain, utest = load_dataset(batch_name, endtag, 1000)
    ...
    for irow, optim in enumerate(["SGD", "Adam"]):
        ...
        for direc in direcs:
            if direc in alldirecs:
                modeloss_data = np.loadtxt("nets/"+direc+"/log_modes.txt")
...
plt.subplots_adjust(wspace=0.02, hspace=0.02, left=0.06, right=0.88, bottom=0.1, top=0.98)

#plt.tight_layout()

fig.savefig("/home/johannes/Nextcloud/Documents/Uni/XI/MA/new2/imgs/sb/bids0346_pink_size"+str(sizeid)+".pdf")
