...
## ---------------------------------------------------------------------------
## MULTISEED VARIANT of analyze_whichTs_newlayout2.py
...
bid   = int(don_code.sys.argv[1]) #3
lrtag = int(don_code.sys.argv[2]) #40
LLW   = int(don_code.sys.argv[3]) #-1
...
def plot_multLLW(showlast=True):
    minval = 100
    maxval = 0
    fig, axs = don_code.plt.subplots(1, 2, figsize=(6,3), sharey=True)
    grand_dict = {}
    band_dict = {}
    n_seeds_used = []
    for whT in all_whTs:
    ...
    axs[1].plot([], [], 'o', linestyle='none', fillstyle='none', markeredgewidth=2, color="gray", label=r"$\delta$", linewidth=3)
    axs[1].plot([], [], '--', color="gray", label=r"$\delta_T$", linewidth=3)
    axs[1].plot([], [], '-', color="gray", label=r"$\delta_B$", linewidth=3)
    for whT in all_whTs:
        ...
        xl = list(xax)
        _ax = axs[plotinfo[whT]["ax"]]
        multiseed.band(_ax, xl, [band_dict[whT][x][0] for x in xl],
                       [band_dict[whT][x][1] for x in xl], plotinfo[whT]["color"])
        multiseed.band(_ax, xl, [band_dict[whT][x][2] for x in xl],
                       [band_dict[whT][x][3] for x in xl], plotinfo[whT]["color"])
        multiseed.band(_ax, xl, [band_dict[whT][x][4] for x in xl],
                       [band_dict[whT][x][5] for x in xl], plotinfo[whT]["color"])

        axs[plotinfo[whT]["ax"]].plot(xax, ete, 'o', linestyle='none', fillstyle='none', markeredgewidth=2, color=plotinfo[whT]["color"], linewidth=3)
        axs[plotinfo[whT]["ax"]].plot(xax, proj_te, '--', color=plotinfo[whT]["color"], linewidth=3)
        axs[plotinfo[whT]["ax"]].plot(xax, bran_te, '-', color=plotinfo[whT]["color"], linewidth=3)
        ...
        axs[1].plot([], [], marker='s', linestyle='none', label=don_code.trunk_names[whT], color=plotinfo[whT]["color"], linewidth=3)
    ...
    for axid in [0,1]:
        axs[axid].tick_params(labelsize=10)
        axs[axid].set_xticks([10, 50, 90])
    
    axs[0].set_ylim((minval/4, maxval*2))
    
    
    #axs[plotinfo[whT]["ax"]].set_xlabel(r"Inner Dimension $N$", fontsize=20)
    fig.text(0.4, 0.02, r"inner dimension $N$", fontsize=10)
    axs[0].set_ylabel(r"relative error $\delta$", fontsize=10)
...
fig.subplots_adjust(wspace=0.0, hspace=0.0, right=0.95, left=0.13, top=0.95, bottom=0.13)

path = don_code.figures_dir
tmp = "Fig2_multiseed" + multiseed.suffix()
