...
fig, axs = plt.subplots(2, len(bids), figsize=(6,4)) #, constrained_layout=True)
...
for ib, bid in enumerate(bids):
    ...
    for irow, optim in enumerate(["SGD", "Adam"]):
        ...
        axs[0,ib].set_title(names[ib], fontsize=fontsize)
    ...
    axs[0,ib].set_xticks([])
    if llw == 20:
        axs[1,ib].set_xticks([0, 7, 15], ["1", "8", "16"])
    else:
        axs[1,ib].set_xticks([0, 19, 39], ["1", "20", "40"])


yfac = 1.5
for ib in range(len(bids)):
    axs[0,ib].set_ylim((min_y_gd/yfac, max_y_gd*yfac))
    axs[1,ib].set_ylim((min_y_adam/yfac, max_y_adam*yfac))
...
axs[1,len(bids)-1].plot([], [], '.-', color="k", label=r"$\sigma_i^{-0.2}$")
#axs[0,len(bids)-1].legend(loc='lower right', fontsize=fontsize)
...
axs[1,len(bids)-1].plot([], [], '--', linewidth=3, color=colors[2], label=r"TV")
axs[1,len(bids)-1].plot([], [], '--', linewidth=3, color=colors[3], label=r"LE")
...
# put legend really in the corner!
axs[1,len(bids)-1].legend(loc="lower right", bbox_to_anchor=(1.05, -0.02), borderpad=0.0, fontsize=fontsize)
#leg=axs[1,len(bids)-1].legend(fontsize=fontsize, bbox_to_anchor=(0.4, 0.6), framealpha=1.0)
#leg.set_in_layout(False)
#fig.canvas.draw()


axs[0,0].set_ylabel(r"GD", fontsize=fontsize)
axs[1,0].set_ylabel(r"Adam", fontsize=fontsize)

fig.text(0.4, 0.04, r"mode index $i$", fontsize=fontsize)
fig.text(0.02, 0.3, r"unweighted mode loss $L_i$", fontsize=fontsize, rotation=90)
fig.text(0.95, 0.3, r"frequency and singular values", fontsize=fontsize, rotation=90)
#fig.suptitle(r"Mode index $i$", fontsize=fontsize, y=0.04)

plt.subplots_adjust(wspace=0.01, hspace=0.01, left=0.15, right=0.88, bottom=0.126, top=0.93)

#plt.tight_layout()

name = "/home/johannes/Nextcloud/Documents/Uni/XI/MA/new2/imgs/sb/bids0346_axis_size"+str(sizeid)
