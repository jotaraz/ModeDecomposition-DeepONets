...
## ---------------------------------------------------------------------------
## MULTISEED VARIANT of show_components_2x2.py
## Every plotted quantity is the ARITHMETIC MEAN over seeds v0..v9.
## log.txt stores log10 values and the figure plots 10**(col/2), so tr_loss and
## te_loss are converted per seed FIRST and averaged afterwards -- averaging the
## raw column would give a geometric mean.  The coupling file
## (log_diagoffdiag_*.txt) and log_modes.txt store raw values, so those are
## averaged directly.  Mode indices are comparable across seeds because the SVD
## basis is data-derived and the data is identical for every seed.
...
for dir in direcs:
    if dir in alldirecs:
        ...
        # convert per seed, then average (arithmetic mean of the plotted error)
        tr_loss, _, _, _ = multiseed.load_mean(
            dir, "log.txt", transform=lambda a: 10**(a[:num_epochs_loss:2,1]/2))
        te_loss, _, _, _ = multiseed.load_mean(
            dir, "log.txt", transform=lambda a: 10**(a[:num_epochs_loss:2,4]/2))
        bigdata, big_lo, big_hi, nseed = multiseed.load_mean(dir, bigname+".txt")
        if bigdata is None:
            print("multiseed: no usable seed for", dir); continue
        print("multiseed: %s -> mean over %d seeds" % (dir, nseed))
        epochs        = bigdata[:,0]
        tr_diags      = bigdata[:,1]
...
for dir in direcs:
    if dir in alldirecs:
        print("YES found", dir)
        modeloss_data, _, _, _ = multiseed.load_mean(dir, "log_modes.txt")
