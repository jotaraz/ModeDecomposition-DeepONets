...
for jdd in range(len(direcs)):
    ...
    if not doneyet and complete and ( stacked_str in dir and nepstr in dir and optstr in dir ) :
        ...
        for epoch in epochs:
            ...
            tmp_str = don_code.nets_dir+"/"+dir+"/"+epoch
            ...
            if "doStackedFalse" in dir:
                ...
                ## compute the unweighted loss components after taking a small step in the direction of each component gradient
                update_matrix_train = don_code.np.zeros((llw, llw))
                update_matrix_test  = don_code.np.zeros((llw, llw))
                ...
                appendix = don_code.np.zeros(4*llw*llw + 2*llw)
                appendix[:llw]      = base_loss_train
                appendix[llw:2*llw] = base_loss_test
                appendix[2*llw+0*llw**2:2*llw+1*llw**2] = update_matrix_train.flatten()
                appendix[2*llw+1*llw**2:2*llw+2*llw**2] = update_matrix_test.flatten()
                appendix[2*llw+2*llw**2:2*llw+3*llw**2] = VTV_train.flatten()
                appendix[2*llw+3*llw**2:2*llw+4*llw**2] = VTV_test.flatten()
        ...
        if "doStackedFalse" in dir:
            diag_data = don_code.np.zeros((len(epochs), 11+2*llw+4*llw*llw))
