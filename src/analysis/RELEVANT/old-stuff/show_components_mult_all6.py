...
line_ids = [0, 1, 4, 12] #, 16]

fig1, axs1 = plt.subplots(1, 3, figsize=(10,5))
dotest = False
if dotest:
    fig2, axs2 = plt.subplots(3, len(line_ids), figsize=(10,8))
else:
    fig2, axs2 = plt.subplots(2, len(line_ids), figsize=(10,6))
...
nt, nb, rtrain, rtest, ptrain, ptest, utrain, utest = load_dataset(batch_name, endtag, num_data)
...
for dir in direcs:
    if dir in alldirecs:
        print("YES found", dir)
        lossdata      = np.loadtxt("nets/"+dir+"/log.txt")
        ...
        for ij,j in enumerate(line_ids):
            base_loss_train          = bigdata[j,9               :9+llw]
            base_loss_test           = bigdata[j,9+llw           :9+2*llw]
            update_matrix_train_flat = bigdata[j,9+2*llw+0*llw**2:9+2*llw+1*llw**2] 
            update_matrix_test_flat  = bigdata[j,9+2*llw+1*llw**2:9+2*llw+2*llw**2]
            VTV_train_flat           = bigdata[j,9+2*llw+2*llw**2:9+2*llw+3*llw**2] 
            VTV_test_flat            = bigdata[j,9+2*llw+3*llw**2:9+2*llw+4*llw**2]
            ...
            #print(ij, j, np.max(VTV_train_flat), np.min(VTV_train_flat))
            #idspos = np.where(VTV_train > 0)
            #print(np.sum(VTV_train[idspos] / len(idspos[0])))
...
for dir in direcs:
    if dir in alldirecs:
        ...
        axs1[1].set_ylabel(r"Loss Change Contributions", fontsize=12)
        ...
        for ij,j in enumerate(line_ids):
            ...
            print(j, np.sum(ss_train[:llw]**2 * base_loss_train))
            
            #plt.figure()
            
            #fig, axs = plt.subplots(2,1)
            #fig.suptitle(dir+"\n"+bigname+"\nEpoch: "+str(epochs[j])+", taylor tr: "+str(np.sum(VTV_train))+", taylor te: "+str(np.sum(VTV_test)))
            '''
            for modeid in modeids:
                ...
                fac = np.max(upd_tr) / np.max(VTV_train[:,modeid])
                ...
                print(np.sum( (upd_tr-scaled_gr_tr)**2 ) / np.sum( (scaled_gr_tr)**2 ))
                print(np.sum( (upd_te-scaled_gr_te)**2 ) / np.sum( (scaled_gr_te)**2 ))
            '''
            #A = np.matmul(update_matrix_train, np.diag(ss_train[:llw]**2)) / (n_train * m_train)
            #B = np.matmul(update_matrix_test, np.diag(ss_train[:llw]**2))/ (n_test * m_test)
            #megamatrix = np.concatenate((A,B), axis=1)
            #fancyshow(megamatrix, axs[0], fig)
            ##axs[1,0].imshow(megamatrix)
            #megamatrix = np.concatenate((VTV_train, VTV_test), axis=1)
            ...
            if np.min(train_modes[2*j,:]) < 1.0:
                a = 0
                #maxid = np.max(np.where( train_modes[2*j, :] < 1.0 )[0]) + 1
                #maxid = np.argmax(ss_train[:llw]**2 * train_modes[2*j, :]/m_train)+1 #np.where( train_modes[2*j, :] < 1.0 )[0]) + 1
...
if not dotest:
    fig2.savefig("/home/johannes/Nextcloud/Documents/Uni/XI/MA/new2/imgs/final_coupl/"+name+"_2.pdf")
